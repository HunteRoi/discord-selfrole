...
export class SelfRoleManager extends EventEmitter {
  ...
  async #sendsMessageAndRegisterChannel(
    ...
    if (selfRoleBotMessages && selfRoleBotMessages.size > 0) {
              ...
              new ActionRowBuilder<ButtonBuilder>().addComponents(
                ...rteData.map((rte: RoleToEmojiData) =>
                  new ButtonBuilder()
                    ...
                    .setCustomId(
                      `${packagePrefix}${rte.role instanceof Role ? rte.role.id : rte.role}`,
                    )
      ...
      if (this.options.useReactions) {
        await Promise.all(
          channelOptions.rolesToEmojis.map((rte) => message?.react(rte.emoji)),
        );
      }
    }
  ...
  async #handleUserAction(
    userAction: ButtonInteraction | MessageReaction | PartialMessageReaction,
    ...
    const memberId = isButtonInteraction && userAction.member ? userAction.member.user.id : user?.id;
    if (!memberId) return;
      ...
      const rolesToRemove = rolesToEmojis.filter(
        (rte) =>
          rte.requiredRoles?.some((role) => {
            return (
              oldMember.roles.resolve(role) && !newMember.roles.resolve(role)
            );
          }) ?? false,
      );
